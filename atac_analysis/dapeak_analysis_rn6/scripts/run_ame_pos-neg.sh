...
# run for positive
ame --evalue-report-threshold 1500 --o ${celltype}_ame_out_with_da_bg_vertebrates_no-thres-pos \
--control ${celltype}-bg_seqs_from_dapeaks.fa ${celltype}-fg_seqs_pos.fa $motifs & \
ame --evalue-report-threshold 1500 --o ${celltype}_ame_out_with_da_bg_vertebrates_no-thres-neg \
--control ${celltype}-bg_seqs_from_dapeaks.fa ${celltype}-fg_seqs_neg.fa $motifs

# message the user on slack if possible
exit_code="$?"
if command -v 'slack' &>/dev/null; then
    if [ "$exit_code" -eq 0 ]; then
		slack "running AME (pos/neg) finished successfully for $celltype" &>/dev/null
	else
		slack "running AME (pos/neg) exited with error code $exit_code for $celltype"
	fi
fi
exit "$exit_code"
